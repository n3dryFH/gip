...
void draw_boxes(Box boxes[])
{
	...
	// Farbe: blue
	// TO DO
	for (unsigned int i = 0; i < box_max; ++i)
	{
...
bool update_boxes(Box boxes[])
{
	...
	for (unsigned int i = 0; i < box_max; ++i)
	{
		Box& box = boxes[i];
		box.y += box.delta_y;
		if (static_cast<unsigned int>(box.y) + static_cast<unsigned int>(box_size) > gip_win_size_y)
			return false;
	}
...
void handle_mouse_click(Box boxes[])
{
	...
	{
		Box& box = boxes[i];
		if (mouse_x >= box.x && mouse_x <= (box.x + box_size) &&
			mouse_y >= box.y && mouse_y <= (box.y + box_size))
		{
			box.delta_y += 10;			
			box.y = 0;
		}
